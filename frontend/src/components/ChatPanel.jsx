...
export default function ChatPanel({ batchId }) {
  ...
  async function send(query) {
    ...
    // After 8s with no response, the backend is likely retrying due to rate limits.
    const slowTimer = setTimeout(() => setLoadingStatus('retrying'), 8000);

    try {
      const { answer, supporting_data } = await askChat(query, batchId);
      clearTimeout(slowTimer);
      setMessages((m) => [...m, { role: 'assistant', text: answer, supporting_data }]);
    } catch (e) {
      clearTimeout(slowTimer);
      const isRateLimit = e.status === 429 || /rate limit|quota|429/i.test(e.message);
      const displayMsg = isRateLimit
        ? '⚠️ Gemini API quota exceeded. Free tier limits: 15 req/min (wait ~60s and retry) or 500 req/day (resets at midnight IST). If retrying in a minute fails, the daily quota is exhausted.'
        : `Error: ${e.message}`;
      setMessages((m) => [...m, { role: 'assistant', text: displayMsg, error: true, isRateLimit }]);
    } finally {
      setLoading(false);
      setLoadingStatus('thinking');
    }
      ...
      <div style={{
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <div style={{
              padding: '10px 16px',
              background: loadingStatus === 'retrying' ? 'rgba(255,149,0,0.06)' : 'var(--surface-2)',
              border: `1px solid ${loadingStatus === 'retrying' ? 'rgba(255,149,0,0.25)' : 'var(--border)'}`,
              ...
              {loadingStatus === 'retrying' ? (
                <span style={{ color: 'var(--orange)', fontSize: 12 }}>
                  ⏳ Rate limited — backend retrying Gemini automatically…
                </span>
              ) : (
                <TypingDots />
              )}
